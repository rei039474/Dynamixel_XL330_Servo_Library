...
void loop() {  
  //// PWM-control (control mode 16) testing with potentiometer 
  //// The value range for XL330 PWM control is -885 ~ 885
  int pinRead = map(analogRead(A0), 0, 1023, -885, 885); // mapping the potentiometer reading to value range of XL330
  robot.setJointSpeed(servoID, pinRead);
  delay(100);
}
